...
for i in range(0, n_folds):
    ...
    anchor_label_list = anchor_label_list_1 + anchor_label_list_2
    
    pos_path_list = pos_path_list_1 + pos_path_list_2
    pos_label_list = pos_label_list_1 + pos_label_list_2
    ...
    anchor_positive_zip = list( zip(anchor_path_list, anchor_label_list, 
                                    pos_path_list, pos_label_list) )

    # Randomly shuffle the zip altogether
    random.shuffle(anchor_positive_zip)

    # Unzip
    anchor_path_list, anchor_label_list, \
        pos_path_list, pos_label_list = zip(*anchor_positive_zip)
    ...
    fold_i_triplets_dataset = tf.data.Dataset.zip(((anchor_dataset, positive_dataset, negative_dataset),
    ...
    fold_i_train_dataset = fold_i_triplets_dataset.map(get_triplet_with_label)

    # Batch the dataset
    fold_i_train_dataset = fold_i_train_dataset.batch(train_batch_size_online)
...
for i in range(0, n_folds):
    ...
    fold_train_datasets[i] = fold_train_datasets[i].map(resize_im_train_online)
...
for i in range(0, n_folds):
    # Map resizing in training dataset for that fold
    fold_train_datasets[i] = fold_train_datasets[i].map(data_aug_faces_online)
...
#       [alpha, l2_pen, batch_all, l_r]

parameter_space = [ 
                    [ 10,  0.1,  True, 1e-5], # Best top-k
                    [ 10, 0.01, False, 1e-6], # Best top-k
                    [ 10,  0.1, False, 1e-4], # Best top-k
                    [0.2,  0.4, False, 1e-6], # Best acc
                    [ 10,  0.2, False, 1e-6], # Best acc
                    [ 20,  0.1,  True, 1e-4], # Best acc
                  ]
...
exp_results = expResults.get_results_from_disk(results_cv_online_file_path,
                                               num_pos_ind_test,
                                               num_neg_ind_test,
                                               face_db_size)
...
for alpha_margin, l2_penalizer, batch_all, learn_rate in parameter_space:
    ...
    expResults.save_results_to_disk(exp_results, results_cv_online_file_path)