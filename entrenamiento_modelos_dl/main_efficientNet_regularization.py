...
embedding = tf.keras.applications.efficientnet.EfficientNetB0(
    include_top=False,
    weights='imagenet',
    pooling=None,
...
retrain_layers_comb = [ ["top_conv"],
                        ["top_conv", "block7"],
                        ["top_conv", "block7", "block6"],
                        ["top_conv", "block7", "block6", "block5"] ]
...
exp_results = expResultsTrad.get_results_from_disk(results_efficentNet_reg,
                                                   size_pos_test,
                                                   size_neg_test,
                                                   face_db_size)
    

print("\n\nEfficientNet top-k validation\n\n")

for learn_rate, retrainables_layers in parameter_space:

    # Model parameters
    parameters_used = "EfficientNet." + \
    ...
    expResultsTrad.save_results_to_disk(exp_results, results_efficentNet_reg)